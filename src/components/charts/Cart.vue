<template>
	<div class="bar-container">
		...
		<v-table class="table">
			...
			<tr>
				<th>Name</th>
				<th v-tooltip="questions.cart">Confidence</th>
				<th>Reason</th>
			</tr>
			</thead>
			<tbody>
			<tr v-for="user in screeningData" :key="user.name">
				...
				<td v-html="user.cart.simplifiedReason"></td>
			</tr>
			<tr>
				<td><b>Average</b></td>
...
const data = ref<ChartData<"bar">>({
	...
	datasets: [
		{
			label: 'Users',
			data: screeningData.map(user => user.cart.confidence),
...
const options = ref<ChartOptions<"bar">>({
	...
	plugins: {
		...
		title: {
			display: true,
			text: 'Confidence Levels in Items in the Cart',
			color: 'white',
			font: {
				size: 24
			}
...
const averageConfidence = computed(() => {
	const totalConfidence = screeningData.reduce((sum, user) => sum + user.cart.confidence, 0);
